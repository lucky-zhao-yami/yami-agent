...
| 类型 | type_id | ✅ | 固定 `6` |
| 标题 | subject | ✅ | User Story 标题 |
| 描述 | description | 否 | Markdown 格式，建议包含：用户故事、验收标准 |
...
```json
{
  "params": {
    ...
    "subject": "作为用户我希望能通过手机号登录",
    "type_id": 6,
    "description": "# 用户故事\n\n作为用户，我希望能通过手机号快速登录，以便减少输入密码的麻烦\n\n# 验收标准\n\n1. 支持手机号+验证码登录\n2. 验证码60秒有效",
...
- User Story 有 6 个必填自定义字段：PIC、Requestor、Channel、Theme、Requirement type、Trimester
- Requestor (customField16) 是 User Story 特有的必填字段，表示需求提出人
- Theme (customField54) 和 Requirement type (customField89) 也是 User Story 特有的必填字段
- Product iteration (customField61) 和 App Version (customField117) 选项值会定期更新，建议创建时通过 `get-schema` 获取最新列表
- description 建议包含：用户故事描述、验收标准
