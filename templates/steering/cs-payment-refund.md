...
## Kibana 日志索引
- 支付服务：`search.py -s ec-payment`，关键词：user_id / purchase_id
- 订单服务：`search.py -s ec-so`，关键词：user_id / purchase_id / order_id
...
### 场景零：添加银行卡失败
触发条件：客人反馈无法添加银行卡、绑卡失败、每次下单都要重新输入卡信息
...
↓
并行查询：风险用户 + 全量日志
├─ crm_risk_user（is_deleted=0）→ 风险用户在 createCardSecret 阶段即被拦截
├─ search.py -s ec-payment -k "user_id" -t 时间范围（全量审查，禁止只查 error 级别）
└─ search.py -s ec-so -k "user_id" -t 时间范围
↓
ec-payment 日志中无该用户记录？
├─ 是 → 请求未到达后端，前端/网络问题
└─ 否 →
      日志中有"风险用户，不返回凭证"？
      ├─ 有 → 被风控拦截（reason=1 为临时邮箱注册），联系风控确认
      └─ 无 →
            日志中有"创建clientSecret"？
            ...
            └─ 有 → SetupIntent 已创建
                  ...
                  │     ├─ card_declined + generic_decline → 发卡行拒绝，建议联系银行或换卡
                  │     └─ 其他 → 根据 Stripe 错误码文档定位
                  └─ 无 → 查是否有"添加卡失败，code:"日志，根据 code 定位
```

日志审查参考关键词：创建clientSecret、SetupIntent、风险用户、setup_failed、添加卡失败、profileId、select
...
↓
数据库无法确定时 → 查日志（search.py -s ec-payment -k "user_id值" -t 7d）
```

### 场景二：支付成功但没有订单（扣款没订单）
触发条件：客人反馈被扣款但没有订单号

排查路径与 cs-order.md 场景三（订单号查询）一致。核心结论：
- 订单存在 → 客人没找到，引导查询
...
│       → 告知客服让用户联系银行确认
└─ 多笔 → [Q2] 查 payment_refund 确认是否已自动退款，未退款需人工处理
↓
数据库无法确定时 → 查日志（search.py -s ec-payment -k "user_id值" -t 7d）
```
...
```
客服提供了什么信息？
├─ 有订单号 → 查 xysc_order_info 获取 purchase_id
├─ 有邮箱 → 执行脚本 `python scripts/get-userid.py "邮箱"` 查 user_id → [Q4] 查最近订单
└─ 有 purchase_id → 直接使用
↓
[Q2] 查 payment_refund
↓
...
│     ├─ status=40（异常）→ 可重试退款，查日志确认原因
│     ├─ status=10（初始化）→ 退款尚未执行，查日志确认是否卡在队列中
│     └─ status=50（失败）→ 查 refund_reason + 日志定位
│           ⚠️ Braintree 信用卡 capture 后超 180 天、微信支付超 1 年，渠道直接拒绝退款（详见 cs-rma.md 场景五）
└─ 无记录 → 查 payment_charge.charge_type：
      ├─ charge_type=0（普通）→ 无退款记录属异常，查日志定位
      └─ charge_type=1（授信）→ 查 payment_charge_order.status：
            ├─ status=50（已取消授权）→ 未实际扣款，银行 hold 金额 3-7 工作日自动释放
            ├─ status=60（已 capture）→ 已扣款但无退款记录，异常，查日志
            ├─ status=51（取消中）→ 等待处理，稍后再查
            └─ status=40（失败）→ 理论上未扣款，查日志确认
```
...
└─ 有 user_id → 直接使用
↓
并行查询：绑定银行卡 + 近期支付记录 + [Q1] 支付尝试日志
↓
所有扣款都能在 xysc_order_info 找到对应订单且为本人下单？
├─ 是 → 排查绑定同一张卡的其他账号
│       → 通过 fingerprint 查所有绑定该卡的账号
│       → 对关联账号执行 `python scripts/get-userid.py "" "user_id"` 查邮箱
│       → NOT_FOUND 时查 xysc_users_delete 表确认是否已删除
│       ↓
│       只有客人本人绑定？
│       ├─ 是 → 扣款都是客人自己的单，告知确认订单记录
│       └─ 否 → 查可疑账号支付记录 + 日志
│             异常信号：绑卡后密集下单 / 多次失败后成功 / 注册时间新但消费异常 / 账号已删除但卡仍活跃
│             ├─ 有异常 → 建议冻结可疑账号，建议客人联系发卡银行挂失换卡
│             └─ 无异常 → 告知客人联系发卡银行挂失换卡
└─ 否（有扣款无对应订单 / 订单非本人 / 金额不符）→ 汇总异常扣款，建议冻结账号
```
...
└─ 有 user_id → 直接使用
↓
查支付预占记录：so_order_purchase_record
↓
status？
├─ status=0（已预占）→ 支付未完成，等待支付完成或超时自动释放
├─ status=1（超时取消）/ status=2（主动取消）→ 资产应已返还
│     ├─ 取消时间距今 < 15 分钟 → 正常延迟，等待
│     └─ > 15 分钟仍未返还 → 按资产类型排查：
│           ├─ 积分 → 查日志（search.py -s ec-so -k "user_id值" -t 7d）
│           ├─ 礼卡 → 查 xysc_egift_log 是否有 reason_flag=2 退回记录，无则查日志
│           └─ 优惠券 → 查 mkt_coupon_code.status 是否恢复为 10，未恢复则查日志
└─ status=3（支付成功）→ 资产已消费，不会返还，属正常
```
...
└─ 有 user_id → 直接使用
↓
查 payment_attempts_log 获取 pay_by_id → 通过 pay_by_id 查所有关联账号
→ 批量执行 `python scripts/get-userid.py "" "user_id"` 查邮箱（xysc_users.email 已脱敏不可用）
→ 已删除账号查 xysc_users_delete 表
↓
风险信号：
- 同一支付账户被多个不相关账号使用
...
### 场景八：退款到账综合排查（礼卡/积分/现金/优惠券）
触发条件：订单取消/RMA 退款后，各资产是否退回到账（已支付成功的场景）

退款执行顺序：退礼卡 → 退积分 → 退现金（优惠券由 central-so-service 单独处理）

```
有订单号 → 查 xysc_order_info 获取 purchase_id 和各资产使用情况
↓
查 fin_receivable 确认退款 job 是否已执行
↓
fin_receivable.status？
├─ status=0（待处理）→ 退款 job 尚未执行，请用户等待
└─ status=1（已处理）→ 查 fin_receivable_detail，按资产类型逐项判断：
      ├─ 现金（amount_type=1）→ [Q2] payment_refund.status=60？
      │     ├─ 是 → 已退款，告知到账时间
      │     └─ 否 → 查日志定位（search.py -s ec-payment -k "purchase_id值" -t 7d）
      ├─ 礼卡（amount_type=2）→ xysc_egift_log 有 reason_flag=2 退回记录？
      │     ├─ 是 → 已退回，查 xysc_egift_card 确认当前余额
      │     └─ 否 → 查日志定位（search.py -s ec-so -k "order_id值" -t 7d）
      ├─ 积分（amount_type=3）→ 退回积分账户（crm_point reason_third=1004002）
      │     注意：下单赠送积分（reason_third=1004001）RMA/整单取消时扣回，补偿退款通常不扣回
      └─ 优惠券（不在 fin_receivable 中）→ mkt_coupon_code.status=10？
            ├─ 是 → 已恢复可用
            └─ 否 → 查日志（search.py -s ec-so -k "order_id值" -t 7d）
```

注意：fin_receivable.status 与 payment_refund.status 可能不同步，两者不一致时以 payment_refund 为准（反映实际退款结果）。
...
### 场景九：地址修改费支付查询
触发条件：客户反馈支付了地址修改费但系统查不到订单（goods_id=2107，固定 $17.00）
...
└─ 有 user_id → 直接使用
↓
查该用户名下是否有 goods_id=2107 的订单 + 对应时间段 $17 的支付记录
↓
用户名下有地址修改费订单？
├─ 有 → 正常流程，查订单状态和支付状态
└─ 无 → 客户可能未登录就支付了
      → 查 payment_charge 中对应时间段所有 $17 的支付记录
      ├─ 找到匹配 → 确认关联的 user_id，可能是游客账户
      └─ 未找到 → 可能走了老版 PHP 网站独立支付通道（不经过 payment_charge）
            → 建议客户提供银行扣款凭证，通过 Stripe Dashboard 反查
```

注意：客户通过 www.yami.com/cn/goods.php?id=2107 支付时，未登录则订单不关联 user_id。
